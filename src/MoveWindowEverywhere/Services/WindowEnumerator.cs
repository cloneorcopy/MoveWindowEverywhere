...
public sealed class WindowEnumerator
{
    ...
    public IReadOnlyList<WindowInfo> Enumerate()
    {
        ...
        foreach (IntPtr handle in seen)
        {
            try
            {
                results.Add(CreateWindowInfo(handle));
            }
            catch (Exception ex)
            {
                // 单个窗口采集失败不应中断整个枚举
                _logger.Warn($"采集窗口 0x{handle.ToInt64():X} 信息失败", ex);
            }
        }
    ...
    /// <summary>读取单个窗口的快照。句柄无效时返回 null。</summary>
    ...
    private WindowInfo CreateWindowInfo(IntPtr handle)
    {
        ...
        return new WindowInfo
        {
            ...
            IconHandle = GetIconHandle(handle),
        };
    }
