...
- 每次打开都会**重新枚举一次**窗口，尽量反映当前状态。枚举分两段：先只做进程内查询把无关窗口淘汰掉，再只为进入列表的窗口补全进程名与图标，因此系统里顶层窗口再多也不会拖慢弹出。
...
现象：从按下快捷键到选择器弹出稳定要等 0.2～0.7 秒，日志里还出现过 2.0 / 2.5 / 3.4 / 6.4 / 6.5 / 7.6 / **10.6 秒**的长尾（判据是程序自己记录的「枚举耗时」，见 `%LOCALAPPDATA%\MoveWindowEverywhere\logs`）。这段时间全部花在 UI 线程上，也就是用户感知的「按了没反应」。

定位（探针实测，一台日常使用的机器，四块屏）：

| 测量项 | 结果 |
| --- | --- |
| `EnumWindows` 返回的顶层句柄 | 1714 个 |
| 通过过滤、真正进入列表的 | 18～21 个 |
| `GetWindowTitle` 走完全部句柄 | 22 ms |
| `WM_GETICON` 跨进程消息走完全部句柄 | 1673 次 / 约 576 ms，单个「线程正忙但未判为未响应」的窗口最多 3×120 ms |
| `Process.GetProcessById` 取进程名 | 缓存未命中时约 11 ms / 唯一 PID，166 个 PID 约 1.9 s |

原因不在于某个 API 特别慢，而在于**顺序**：`WindowEnumerator` 先给每个句柄补齐全部字段，才交给 `WindowFilterPolicy` 淘汰，于是 98.8% 的采集浪费在根本不会显示的窗口上，而最贵的两项（图标要发消息、进程名要开进程句柄）恰好摊得最广。

修复：

- 枚举改为两段式：先只做进程内查询（可见性、样式、类名、标题、DWM cloak、所在显示器）并用**同一个** `WindowFilterPolicy` 淘汰，之后只为幸存窗口补 `ProcessName` 与 `IconHandle`。过滤规则仍然单点保存在 `WindowFilterPolicy`，没有第二份判定逻辑。
- 图标索取的单次超时由 120 ms 收到 30 ms。健康的窗口处理这类查询只用微秒级，超时只对线程正忙却又没被 `IsHungAppWindow` 判为未响应的窗口生效，代价仅是拿不到窗口自设的图标、退回窗口类图标显示。
- 新增行为等价测试：两段式的结果必须与「先全量采集再过滤」给出同一批窗口。同时把「枚举必须在有限时间内返回」的时间预算由 10 s 收紧到 1.5 s——旧预算恰好放行了上面那次 10.6 秒。

效果（直接调用改动后的真实代码，同一台机器同一批窗口，两条路径都产出 18 行）：

| 路径 | 冷启动（进程名缓存为空） | 稳态 |
| --- | --- | --- |
| 旧：`policy.Apply(Enumerate())` | 933 / 993 / 1081 / 1567 ms | 303 / 365 / 471 ms |
| 新：`Enumerate(policy)` | 11 / 14 / 26 / 304 ms | 15 / 59 / 144 ms |

尚未处理的两项：长尾仍受「幸存窗口里恰好有忙线程」影响，最坏 18×3×30 ms；选择器窗口每次触发都重建，且在 `ContentRendered` 之后才定位，会先在错误位置闪一下再跳走。后者属可见行为变更，需单独确认后再改。

发布版端到端复核（重新 `dotnet publish` 后实际启动，判据仍是程序自身日志）。启动后第一次触发即冷路径（进程名缓存为空）：

```text
[21:44:23] [INFO] 启动 Move Window Everywhere（PID 89172，.NET 8.0.29）
[21:44:24] [INFO] 已注册全局快捷键 Alt + Z
[21:45:28] [INFO] 触发来源：第二实例唤醒
[21:45:28] [INFO] 已打开窗口选择器：目标显示器 显示器 2…，候选窗口 19 个，显示器 4 台，枚举耗时 35 ms
```

同一天旧版记录的是 188 / 209 / 290 / 315 ms（那次候选窗口 16 个），闲置九小时后的首次触发是 1398 ms。
