...
**无法自动验证、必须手动确认的部分见下一节。**

---

## 十二、手动测试清单

以下场景涉及真实多显示器 / DPI / 权限环境，需人工执行。建议每项记录「通过 / 失败 + 现象」。

### 1. 双显示器基础场景

- [ ] 两台分辨率不同的显示器，鼠标放在 **副屏**，按快捷键：选择器出现在副屏，选中的窗口移到副屏。
- [ ] 鼠标放在 **主屏**，按快捷键，选中一个原本在副屏的窗口：窗口移回主屏。
- [ ] 目标显示器编号提示与实际显示器一致。
- [ ] 选择器列表中每个窗口右侧的「屏 N」标签与窗口**实际所在显示器**一致；已在目标显示器上的窗口标签用强调色显示。
- [ ] 把窗口手动拖到另一块屏后重新打开选择器，标签随之更新。
- [ ] **单显示器**接驳时（拔掉副屏或只启用一块），列表中不显示任何「屏 N」标签。
- [ ] 打开选择器后列表**立即出现**（先看到骨架与图标），缩略图随后逐张浮现，不出现整体卡顿。
- [ ] 缩略图内容与对应窗口一致（不是别的窗口的样子、不是上下颠倒）。
- [ ] 最小化窗口、已挂起的程序显示「无预览」而不是一块黑。

### 2. 负坐标显示器

- [ ] 把副屏拖到主屏**左侧**（副屏坐标为负），重复上述移动，窗口正确定位，不跑到屏幕外。
- [ ] 把副屏拖到主屏**上方**，重复验证。
- [ ] 上下排列时不发生窗口跑到主屏下方的情况。

### 3. DPI 缩放差异

- [ ] 主屏 100%、副屏 150% 缩放，从 100% 屏移到 150% 屏：窗口居中，无偏移、无尺寸错乱。
- [ ] 主屏 150%、副屏 100%，反向验证。
- [ ] 两块屏幕都设 125% 后再验证一次。

### 4. 不同任务栏位置

- [ ] 副屏任务栏在底部：窗口居中区域避开任务栏（居中于 `rcWork` 而非整屏）。
- [ ] 副屏任务栏在左侧或顶部，重复验证窗口不会压到任务栏。
- [ ] 任务栏设为「自动隐藏」后再次验证。

### 5. 窗口状态

- [ ] **普通窗口**：移动后保持原尺寸并居中。
- [ ] **最大化窗口**：移动后在目标显示器上仍然最大化。
- [ ] **最小化窗口**：移动后恢复为可见普通窗口并居中（与文档约定一致）。
- [ ] 尺寸**大于**目标工作区的窗口（例如把 4K 上全屏的窗口移到 1080p 屏）：尺寸被限制在工作区内，不超出屏幕。
- [ ] **无边框**窗口（如浏览器无边框模式、部分工具窗口）：能正常移动。

### 6. 权限

- [ ] 以**普通权限**运行，尝试移动**任务管理器**（以管理员运行）：出现「权限高于本程序」的明确提示，程序不崩溃、托盘图标仍在。
- [ ] 以**管理员身份**运行本工具，再次移动任务管理器：成功移动。
- [ ] 移动一个内部会自我修正位置的程序（如某些播放器）：观察是否被夹紧回目标工作区。

### 7. 快捷键

- [ ] 默认 `Alt + Z` 可用时正常触发。
- [ ] 人为制造冲突（先用其他程序占用某组合），在设置里改到该组合：出现「已被其他程序占用」提示，原快捷键仍然可用。
- [ ] **自动换用**：先用别的程序占用 `Alt + Z`，再启动本程序：日志出现「快捷键 Alt + Z 不可用，已自动改用 …」，托盘气泡提示新组合，新组合按下去有效，配置文件里的 `Hotkey` 已更新为新值。
- [ ] **自动换用后重启**：直接使用上次换用的组合，日志中不再出现失败与换用记录。
- [ ] **关闭自动换用**后再制造冲突：不换用，出现「快捷键未生效」气泡提示，程序仍常驻托盘。
- [ ] 保存新快捷键并**重启程序**：新快捷键仍然生效（配置已持久化）。
- [ ] 在设置窗口点「取消」后：原快捷键仍可用（未丢失）。
- [ ] 只按普通键（不加修饰键）时提示无效。

### 8. 设置窗口

- [ ] 托盘菜单显示「设置…」，打开后包含快捷键区与全部行为开关，勾选状态与当前配置一致。
- [ ] 只改动开关、不改快捷键，点保存：立即生效（例如取消勾选「包含最小化窗口」后，列表里不再出现最小化窗口）。
- [ ] 点取消：所有开关的改动都不生效，配置文件内容不变。
- [ ] 取消勾选「显示缩略图」并保存：再次打开选择器，缩略图整列消失、列表更紧凑。
- [ ] 取消勾选「选择器失去焦点时自动关闭」并保存：点击选择器之外的区域，选择器仍然停留，需要 `Esc` 才关闭。
- [ ] 勾选「随 Windows 启动」并保存：注册表 `HKCU\Software\Microsoft\Windows\CurrentVersion\Run` 下出现 `MoveWindowEverywhere` 值，内容为带引号的当前 exe 路径；取消勾选并保存后该值消失。
- [ ] 勾选「随 Windows 启动」后**注销并重新登录**：程序自动启动并在托盘出现。
- [ ] 勾选开机自启后把 exe 移到另一个目录再启动：日志显示开机自启已就绪，注册表里的路径被自动更新为新路径。
- [ ] 通过 `dotnet run` 启动时：开机自启开关为禁用状态并给出说明文字。

### 9. 托盘与生命周期

- [ ] 启动后无主窗口，托盘出现图标。
- [ ] 重复双击 exe：不产生第二个实例，已有实例被唤醒或弹出「已在运行」提示。
- [ ] 托盘菜单每一项均可用，能打开配置目录。
- [ ] 从托盘退出后：托盘图标消失，任务管理器中无残留进程，快捷键被释放（退出后按快捷键无任何反应）。
- [ ] 退出后立即重新启动：单实例互斥量已正确释放，可以正常启动。
- [ ] 连续快速按快捷键 10 次：每次都能打开选择器，不出现界面卡死或残留的隐藏窗口。

### 10. 选择器交互

- [ ] 打开后搜索框自动获得焦点，直接键入即过滤。
- [ ] `↑` / `↓` 切换选中项，列表自动滚动到可视区域。
- [ ] `Enter` 确认并移动；鼠标双击确认并移动。
- [ ] `Esc` 取消：**不移动任何窗口**。
- [ ] 点选择器之外的区域（失焦）：选择器关闭且不移动任何窗口。
- [ ] 搜索无结果时显示空列表提示。
- [ ] 目标窗口在选择过程中被**手动关闭**，再按 `Enter`：出现「已关闭或句柄失效」提示，程序不崩溃。
- [ ] 选择器本身不出现在窗口列表中。
- [ ] 从快捷键触发的瞬间把鼠标快速移开（移到另一块屏），窗口仍移动到**触发瞬间**那台显示器。
- [ ] 列表有几十个窗口时，滚动仍流畅（缩略图在后台加载，不应阻塞滚动）。
...
- 多显示器 / 不同 DPI / 负坐标显示器等场景需要真实多屏环境，由第十二节手动测试清单覆盖。
- 单实例唤醒（第二个实例唤起已有实例）在本机验证环境中后台进程会被冻结，未能端到端验证；消息分发链路已由单元测试覆盖（见第十节第 4 点）。
- 缩略图的实际观感（内容是否正确、四种退回「无预览」的情形是否都符合预期）需要真实桌面环境，已列入手动测试清单第 1 节。
- 开机自启的「注销后自动启动」这一环需要重新登录才能确认真实生效，已列入手动测试清单第 8 节；本轮只验证了注册表位置可写、程序写入逻辑有 12 个用例覆盖。
- 快捷键冲突自动换用的真实触发需要另一个程序先占用组合，已列入手动测试清单第 7 节。
